...
public class dbindexquery {
   ...
   //This program inputs heap file from index file and performs required operation
	...
	//This method fetches the metadata details of the file and key
	private static String fetchMetadata(String indexpath, String command) throws IOException 
	{

		if(command == "file") 
		{
			RandomAccessFile randomAccessFile1 = new RandomAccessFile(indexpath, "r");
			byte[] inputFB = new byte[256];
			randomAccessFile1.read(inputFB);
			String inputFN = new String(inputFB);
			randomAccessFile1.close();
			return inputFN.trim();
		}
		else 
		{
			RandomAccessFile randomAccessFile1 = new RandomAccessFile(indexpath, "r");
			byte[] key = new byte[3];
			randomAccessFile1.seek(257l);
			randomAccessFile1.read(key);
